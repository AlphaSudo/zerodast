...
AUTH_RESPONSE_TOKEN_FIELD="${AUTH_RESPONSE_TOKEN_FIELD:-access_token}"
AUTH_USERNAME_FIELD="${AUTH_USERNAME_FIELD:-username}"
...
extract_token() {
  ...
  if [[ -x "$NODE_BIN" ]]; then
    RESPONSE_JSON="$response" TOKEN_FIELD="$AUTH_RESPONSE_TOKEN_FIELD" "$NODE_BIN" -e "const raw = process.env.RESPONSE_JSON || ''; const field = process.env.TOKEN_FIELD || 'access_token'; try { const parsed = JSON.parse(raw); process.stdout.write(parsed[field] || ''); } catch { process.exit(1); }"
    return
  fi
...
login_and_extract() {
  ...
  response=$(curl -sS "$APP_URL$AUTH_LOGIN_PATH" \
    ...
    --data-urlencode "${AUTH_USERNAME_FIELD}=${email}" \
    --data-urlencode "${AUTH_PASSWORD_FIELD}=${password}")
...
echo "Form-urlencoded token auth adapter bootstrap complete"
