...
$outDir = 'C:\Java Developer\DAST\benchmarks\fullstack-fastapi-template\out\t1'
...
$requestSeeds = @(
  "$scannerTargetUrl/login/test-token",
  "$scannerTargetUrl/users/me",
  "$scannerTargetUrl/items/",
  "$scannerTargetUrl/users/"
)
$containerName = 'fullstack-fastapi-t1-zap'
...
function Write-AutomationConfig {
  ...
  $requestorBlocks = foreach ($url in $requestSeeds) {
@"
  - type: requestor
    parameters:
      user: ""
...
env:
  contexts:
    - name: "fullstack-fastapi-t1"
      urls:
        - "$ScannerBaseRoot"
      includePaths:
        - "$ScannerBaseRoot$ApiBasePath.*"
  parameters:
    failOnError: true
...
jobs:
  ...
  - type: openapi
    parameters:
      ...
      context: "fullstack-fastapi-t1"
$($requestorBlocks -join "`n")
  - type: spider
    parameters:
      context: "fullstack-fastapi-t1"
      url: "$scannerTargetUrl/docs"
      maxDuration: 1
...
$summaryLines = @(
  '# full-stack-fastapi-template T1 Summary',
  '',
